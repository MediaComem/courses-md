 ...
 * `--base-url <url>`            | `$COURSES_MD_BASE_URL`         | `baseUrl`         | Base URL at which the published slides will be available (defaults to "/")
 ...
 * `--publish-git-url <url>`     | `$COURSES_MD_PUBLISH_GIT_URL`  | `publish.gitUrl`  | Git URL of the repository to push the HTML slides to
 * `--publish-base-url <url>`    | `$COURSES_MD_PUBLISH_BASE_URL` | `publish.baseUrl` | If set, overrides the base URL when using the publish command
...
class Config {
  ...
  get effectiveBaseUrl() {
    const baseUrl = this.publishing ? this.publish.baseUrl || this.baseUrl : this.baseUrl;
    return baseUrl.match(/\/$/) ? baseUrl : `${baseUrl}/`;
  }
  ...
  setEnvironmentOptions() {
    return this.set({
      ...
      publish: {
        gitUrl: this.getEnvVar('PUBLISH_GIT_URL'),
        baseUrl: this.getEnvVar('PUBLISH_BASE_URL'),
  ...
  validate() {
    ...
    this.validateString('publish.gitUrl', false);
    this.validateString('publish.baseUrl', false);
    this.validateString('publish.version', false);
  ...
  validateString(property, required = true) {
    if (!required && !_.has(this, property)) {
      return;
    }
