...
exports.compileSlides = async function(config, relativeFilePath) {
  ...
  config.logger.debug(`${chalk.yellow(sourcePath)} -> ${chalk.green(targetPath)}`);
};
...
async function markdownToSlides(config, relativeFilePath) {
  ...
  const templateFunc = await getRemarkPageTemplate();
...
async function getRemarkPageTemplate() {
  if (!templatesCache.remark) {
    templatesCache.remark = handlebars.compile(await fs.readFileSync(path.join(__dirname, 'templates', 'remark.html'), { encoding: 'utf8' }));
  }

  return templatesCache.remark;
}
