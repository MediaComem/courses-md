...
exports.compileSlides = async function(config, relativeFilePath, slidesBundlePath) {

  const compiled = await markdownToSlides(config, relativeFilePath, slidesBundlePath);
...
async function markdownToSlides(config, relativeFilePath, slidesBundlePath) {
  ...
  const remarkPage = templateFunc({
    ...
    slidesBundlePath: path.join(basePath, path.relative(config.absoluteBuildDir, slidesBundlePath)),
